...
- (std::shared_ptr<pitch::Document>)open:(pitch::Analysis)analysis identity:(NSString*)identity sourceHash:(NSString*)hash {
    ...
    if([value isKindOfClass:NSDictionary.class]&&([value[@"schema"] isEqual:@1]||[value[@"schema"] isEqual:@2]||[value[@"schema"] isEqual:@3]||[value[@"schema"] isEqual:@4]||[value[@"schema"] isEqual:@5])&&[value[@"identity"] isEqual:identity]&&[value[@"sourceHash"] isEqual:hash]&&[value[@"notes"] isKindOfClass:NSArray.class]){
        std::vector<pitch::Note> restored;std::set<std::size_t> ids;bool valid=[value[@"notes"] count]<=10000;
        double end=0;
        for(id row in value[@"notes"]){
            if(!valid||![row isKindOfClass:NSArray.class]||[row count]!=([value[@"schema"] isEqual:@5]?10:[value[@"schema"] isEqual:@4]?9:[value[@"schema"] isEqual:@3]?7:[value[@"schema"] isEqual:@2]?6:5)){valid=false;break;}
            ...
            n.formant=[row count]==10?[row[9] doubleValue]:0;
            if(std::abs(n.formant)>6||std::abs(n.driftStart)>200||std::abs(n.driftEnd)>200||n.vibrato<0||n.vibrato>1||n.gainDb < -24||n.gainDb > 12||n.start<end||n.end<=n.start||n.end>analysis.duration+1e-6||n.originalMidi<0||n.originalMidi>127||std::abs(n.semitones)>24||!ids.insert(n.id).second){valid=false;break;}
...
- (void)save:(std::shared_ptr<pitch::Document>)document identity:(NSString*)identity sourceHash:(NSString*)hash {
    ...
    for(const auto& n:document->analysis().notes)[rows addObject:@[@(n.id),@(n.start),@(n.end),@(n.originalMidi),@(n.semitones),@(n.gainDb),@(n.vibrato),@(n.driftStart),@(n.driftEnd),@(n.formant)]];
    NSDictionary *snapshot=@{@"schema":@5,@"identity":identity,@"sourceHash":hash,@"notes":rows};
